...
- **`scan_rotate_node`**, correcting a lidar mounted at a yaw to the chassis. The RPLIDAR on the reference car is 180 degrees out: an object off the car's right nose reads on the left rear of the raw scan, verified on hardware by placing an object to the right (reported at -90 degrees in the dashboards' convention) and in front (reported at 180 degrees). Only a yaw explains both; a mirrored scan would have put the front object at 0, which is why `sllidar`'s `inverted` flag is the wrong tool, and sllidar has no angle-offset parameter. Off by default: `racecar launch lidar scan_rotate:=true` puts sllidar on `/scan_raw` and the rotator on `/scan`. The rotation moves `angle_min` and `angle_max` and leaves the ranges array untouched, so consumers that derive an index from `angle_min` (the lab dashboards) are corrected while consumers that index the array directly (`racecar-neo-library`, via `np.flip`) are unaffected and need no change. Measured on a Pi 5: 27 us per scan to transform and 0.7 ms median for the extra publish hop, against a 100 ms scan period.
